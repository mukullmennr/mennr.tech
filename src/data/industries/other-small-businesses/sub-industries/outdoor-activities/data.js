export const breadcrumb = {
    home: {
        text: "Who We Help",
        link: "/who-we-help",
    },
    industry: {
        text: "Other Small Businesses",
        link: "/who-we-help/other-small-businesses",
    },
    our: "Outdoor Activities",
    next: {
        text: "Hotels",
        link: "/who-we-help/other-small-businesses/hotels",
    },
    prev: {
        text: "Restaurants",
        link: "/who-we-help/other-small-businesses/restaurants",
    },
};

export const main = {
    heading:
        "Why is Mennr the trusted marketing firm for Outdoor Activities Businesses in New England?",
    img: "/common/chipy/sub-industries/other-small-businesses/outdoor-activities.svg",
    showCertification: true,
};

export const faq = {
    heading: "Marketing FAQ's for Outdoor Activities Business",
    data: [
        {
            question: " How do you market outdoor activities?",
            answer: "Effectively marketing outdoor activities involves a diverse approach to engage the target audience. Utilizing various channels such as social media, email marketing, and print advertising can enhance visibility. Partnering with influencers who share a passion for outdoor activities, offering promotions, and forming local partnerships contribute to a robust strategy. Hosting or sponsoring events, both online and offline, helps build community engagement. Content marketing, sharing valuable information, and utilizing customer testimonials add authenticity to your marketing efforts. By employing these strategies collectively, you can effectively promote your outdoor activities, attract a wider audience, and drive participation.",
        },
        {
            question: "What are the strategies for Home Services Marketing?",
            answer: "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Morbi pulvinar sed libero et rhoncus. Etiam varius tincidunt velit, quis dapibus enim dapibus eu. Aliquam erat volutpat. Vivamus dictum, erat in finibus egestas, quam tortor malesuada ipsum, quis porta risus eros non elit. ",
        },
        {
            question: "How will Home Services marketing help your business?",
            answer: "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Morbi pulvinar sed libero et rhoncus. Etiam varius tincidunt velit, quis dapibus enim dapibus eu. Aliquam erat volutpat. Vivamus dictum, erat in finibus egestas, quam tortor malesuada ipsum, quis porta risus eros non elit. ",
        },
        {
            question: "How do you generate  Home Services Customers?",
            answer: "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Morbi pulvinar sed libero et rhoncus. Etiam varius tincidunt velit, quis dapibus enim dapibus eu. Aliquam erat volutpat. Vivamus dictum, erat in finibus egestas, quam tortor malesuada ipsum, quis porta risus eros non elit. ",
        },
    ],
};

export const steps = {
    steps: [
        "Seeing how you compare to other Outdoor Activities businesses in your area is easy",
        "Open a new tab in your browser, and type the following line into your search bar: ‘outdoor activities near me’",
        `You’ll notice 3 things:\n
1. Sponsored websites (paid)
2. Map showing all relevant businesses (free)
3. Where your website appears on the page (free)`,
        "These 3 things are how potential customers find you through Google",
        "60% of people searching for businesses on Google only look at the first 3 recommend results",
        " If you’re not ranking in the top 3, you’re losing out on  business, simple as that",
        "But don’t fret, with time and the right strategy, appearing as one of the top 3 recommended businesses isn’t rocket science",
    ],
    last: "Talk with one of our Google Certified marketing specialists today, find a marketing strategy that works best for you and your Outdoor Activities business",
};

export const quote =
    "Apart from experience, our ability to turn struggling businesses into sustaining ones helps Outdoor Activities Businesses in New England stay operational for years to come.";
